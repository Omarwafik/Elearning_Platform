...
async function displayCourses(courses) {
  if (!courses || !Array.isArray(courses)) {
    container.innerHTML =
      '<div class="text-danger fs-3 p-4">⚠️ Error loading courses. write this in terminal json-server --watch db.json --port 3001</div>';
    return;
  }
  let element = "";
  courses.forEach((i) => {
    element += `
        ...
        <img id="course-image" src="${i.img}" alt="">
         ...
         <p ><strong>Instructor:</strong> <span id="course-instructor">${i.instructor}</span></p>
            <div class="bottom d-flex justify-content-between  align-items-center mt-auto">
...
function search() {
  ...
  if (filtered.length === 0) {
    container.innerHTML =
      '<div class="no-results text-light p-5  fs-5">No courses found matching your search</div>';
    return;
  }
